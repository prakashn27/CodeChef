...
int main(int argc, char* argv[])
{
	...
	for(;;)
	{
		...
		__in >> __expected >> players;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << players << std::endl;
		std::cout.flush();

		__timer_start();

		PaperRockScisQuals __object;
		int __actual = __object.whoPassed(players);
