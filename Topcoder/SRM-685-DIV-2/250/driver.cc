...
#include "MultiplicationTable2Easy.cc"
...
vector <int>	t;
		...
		__in >> __expected >> table >> t;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << table << ", " << t << std::endl;
		std::cout.flush();

		__timer_start();

		MultiplicationTable2Easy __object;
		string __actual = __object.isGoodSet(table, t);
