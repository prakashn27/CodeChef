...
vector <int>	grid;
		...
		__in >> __expected >> n >> m >> grid;
		if (!__in.good()) break;

		std::cout << "# input for test " << __testnum << ": " << n << ", " << m << ", " << grid << std::endl;
		std::cout.flush();

		__timer_start();

		GridSortMax __object;
		string __actual = __object.findMax(n, m, grid);
